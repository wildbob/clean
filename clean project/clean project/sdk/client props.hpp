...
#define SPROP_COORD				(1<<1)	// If this is set, the float/vec3_t is treated like a world coordinate.
...
#define SPROP_NORMAL			(1<<5)	// If this is set, the vec3_t is treated like a normal (only valid for vec3_ts)

#define SPROP_EXCLUDE			(1<<6)	// This is an exclude prop (not excludED, but it points at another prop to be excluded).

#define SPROP_XYZE				(1<<7)	// Use XYZ/Exponent encoding for vec3_ts.
...
#define SPROP_IS_A_vec3_t_ELEM	(1<<11)	// Set automatically if SPROP_vec3_tELEM is used.
...
#include <stdio.h>
		...
		const char *ToString()
		{
			...
			case DPT_vec3_t:
				sprintf_s(text, sizeof(text), "(%.3f,%.3f,%.3f)",
					m_vec3_t[0], m_vec3_t[1], m_vec3_t[2]);
				break;
#if 0 // We can't ship this since it changes the size of DTVariant to be 20 bytes instead of 16 and that breaks MODs!!!
			case DPT_Quaternion:
				sprintf_s(text, sizeof(text), "(%.3f,%.3f,%.3f %.3f)",
					m_vec3_t[0], m_vec3_t[1], m_vec3_t[2], m_vec3_t[3]);
				break;
#endif
			case DPT_String:
				if (m_pString)
	...
	public:
		...
		RecvVarProxyFn m_vec3_tTovec3_t;
	};
...
#define RECVINFO_DTNAME(varName,remoteVarName)	#remoteVarName, offsetof(currentRecvDTClass, varName)
	...
	void RecvProxy_vec3_tTovec3_t(const CRecvProxyData *pData, void *pStruct, void *pOut);
	...
	RecvProp RecvPropvec3_t(
		const char *pVarName,
		int offset,
		int sizeofVar = SIZEOF_IGNORE,	// Handled by RECVINFO macro, but set to SIZEOF_IGNORE if you don't want to bother.
		int flags = 0,
		RecvVarProxyFn varProxy = RecvProxy_vec3_tTovec3_t
	);

	// This is here so the RecvTable can look more like the SendTable.
#define RecvPropangle_ts RecvPropvec3_t
