...
//modified:
//	added Vars.Menu.bOpened check to color picker
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	ImVec2 Newgrab;
	Newgrab.x = frame_bb.Min.x + 2;
	Newgrab.y = frame_bb.Min.y + grab_padding; //pickles
	window->DrawList->AddRectFilled(Newgrab, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		if (!(flags & ImGuiColorEditFlags_NoOptions) && IsItemHovered() && IsMouseClicked(1) && Vars.Menu.bOpened)
			OpenPopup("context");
	}
